...
export default function HotRightNow() {
  ...
  useEffect(() => {
    const unsub = subscribeToProducts((data) => setProducts(data), null, null, 'regular');
    return () => unsub();
  }, []);
    ...
    <section className="ecom-section ecom-hot">
      <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div className="section-subtitle" style={{ display: 'flex', alignItems: 'center', gap: '1rem', margin: 0, fontWeight: 700, fontSize: '0.75rem', letterSpacing: '0.1em' }}>
          <span style={{ color: '#FF5B22', textTransform: 'uppercase' }}>FROM TODAY'S STREAMS</span>
          <a href="#" style={{ color: '#1a1a1a', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '4px' }}>VIEW ALL <ArrowRight size={14}/></a>
        </div>
        <h2 className="section-title serif-heading" style={{ margin: 0, fontSize: '1rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Hot Right Now</h2>
      </div>

      <div className="hot-grid-v2">
        {displayProducts.length === 0 && <p style={{opacity: 0.5}}>No products matching this criteria.</p>}
        {displayProducts.map((item, idx) => {
          const bgColors = ['#f4efe8', '#e8f0f4', '#f4e8e8', '#e8f4ec'];
          const badgeColors = ['#FF5B22', '#3b82f6', '#FF5B22', '#10b981'];
